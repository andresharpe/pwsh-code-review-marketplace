...
When the project renders prompts (or any text) by `-replace`-ing uppercase placeholder tokens (regex `\{\{[A-Z][A-Z0-9_]*\}\}`) in markdown, the reviewer flags two failure classes under `PWSH-TPL-NNN`. The token catalog is auto-discovered from `-replace`-pattern occurrences whose left-hand side matches that shape, and snapshotted to `.pwsh-review/template-rules.json` at bootstrap. The lowercase / hyphenated placeholder names that appear in this section (e.g. `{{token-name}}`) are deliberately out of the rule's regex so that the documentation can demonstrate the prose-pattern checks without tripping the rule on its own description.

- `PWSH-TPL-001` - the markdown references an uppercase placeholder that has no `-replace` rule anywhere in the repo. The runtime will leave the literal placeholder in the rendered output. Either fix the typo or add a substitution rule.
- `PWSH-TPL-002` - prose conditional that the substitution will never satisfy: text near an uppercase placeholder reference says, e.g., "if `{{token-name}}` is empty / missing / blank / absent / not set". When the placeholder is provably always non-empty (the `-replace` source declares an explicit fallback), the conditional is dead code (confidence 80). When the placeholder's emptiness is uncertain, the finding is hedged (confidence 60).
